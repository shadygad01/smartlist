...
export default function ArchivePage() {
  ...
  // Derive available sectors from production data — no hardcoding
  ...
  const avgRet   = events.length > 0
    ? events.reduce((s, e) => s + e.return_pct, 0) / events.length
    : 0;
  const peakBest = events.length > 0
    ? Math.max(...events.map((e) => e.peak_return_pct))
    : 0;

  // Unique statuses from production data
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--background)', paddingTop: '72px' }}>
      ...
      <div className="px-4 md:px-6 xl:px-10 py-6 space-y-6">
        ...
        {/* KPIs — from production snapshot statistics */}
