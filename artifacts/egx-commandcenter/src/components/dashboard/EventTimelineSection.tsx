...
import {
  Clock, TrendingUp, RefreshCw, AlertTriangle, Settings,
  ChevronDown, ChevronUp, Filter, Circle,
} from 'lucide-react';
...
const EVENT_CONFIG: Record<EventType, {
  ...
  BUY_SIGNAL: { dot: '#4caf50', badge: 'BUY SIGNAL', badgeBg: 'rgba(76,175,80,0.15)', badgeText: '#4caf50', tickerColor: '#ffffff', Icon: TrendingUp },
  NEAR_ENTRY: { dot: '#f5a623', badge: 'NEAR ENTRY', badgeBg: 'rgba(245,166,35,0.15)', badgeText: '#f5a623', tickerColor: '#ffffff', Icon: Clock },
  SCAN: { dot: '#4a9eff', badge: 'SCAN', badgeBg: 'rgba(74,158,255,0.12)', badgeText: '#4a9eff', tickerColor: '#4a9eff', Icon: RefreshCw },
  VOL_SPIKE: { dot: '#f5a623', badge: 'VOL SPIKE', badgeBg: 'rgba(245,80,30,0.15)', badgeText: '#f5803e', tickerColor: '#ffffff', Icon: AlertTriangle },
  SYSTEM: { dot: '#6b7280', badge: 'SYSTEM', badgeBg: 'rgba(107,114,128,0.12)', badgeText: '#8b8fa8', tickerColor: '#8b8fa8', Icon: Settings },
};
  ...
  return (
    <span
      style={{
        ...
        backgroundColor: '#4caf50', boxShadow: '0 0 0 0 rgba(76,175,80,0.6)',
        animation: 'etl-pulse 2s infinite', flexShrink: 0,
      }}
    />
  );
}
...
function EventRow({ evt }: { evt: EventTimelineItem }) {
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3" style={{ borderBottom: '1px solid rgba(37,38,69,0.7)' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          {!isScan && !isSystem && evt.ticker && (
            <span className="font-mono font-bold" style={{ fontSize: '12px', color: cfg.tickerColor }}>
              {evt.ticker}
            </span>
          )}
          <span
            ...
            style={{
              fontSize: '9px', backgroundColor: cfg.badgeBg, color: cfg.badgeText,
              border: `1px solid ${cfg.badgeText}44`, letterSpacing: '0.05em', flexShrink: 0,
            }}
          >
            {evt.badge || cfg.badge}
          </span>
        ...
        {evt.description && (
          <div className="font-mono mt-0.5" style={{ fontSize: '11px', color: '#8b8fa8', lineHeight: 1.4 }}>
            {evt.description}
          </div>
        )}
      </div>
      <div className="flex-shrink-0 flex flex-col items-end gap-0.5" style={{ paddingTop: 2 }}>
        <span className="font-mono" style={{ fontSize: '11px', color: '#6b7280' }}>{wallTime(evt.timestamp)}</span>
        <span className="font-mono" style={{ fontSize: '9px', color: '#4a4d6a' }}>{wallDate(evt.timestamp)}</span>
      </div>
    </div>
  );
}
      ...
      <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#10112a', border: '1px solid #252645' }}>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                fontSize: '10px', minWidth: 22, height: 22, padding: '0 6px',
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(37,38,69,0.8)', cursor: 'pointer',
              }}
              title="Filter events"
            >
        ...
        {!collapsed && (
          <div>
            ...
            <div className="flex items-center gap-2 px-4 py-2" style={{ borderTop: '1px solid rgba(37,38,69,0.5)' }}>
              <Circle
                size={6}
                style={{ color: lastTs ? '#4caf50' : '#6b7280', fill: lastTs ? '#4caf50' : '#6b7280', flexShrink: 0 }}
              />
              <span className="font-mono" style={{ fontSize: '10px', color: '#6b7280', letterSpacing: '0.04em' }}>
