"""
EGX SMC Scanner - 5-Year Backtesting with REGIME-AWARE Detection
Market Regime Optimization | Dynamic Parameters | Risk Management

استراتيجية SMC مع كشف أنماط السوق (Regime-Aware)
"""
...
def detect_market_regime(close, high, low, volume, period_short=20, period_mid=50, period_long=200):
    """
    كشف نمط السوق (Market Regime)
    
    العائد:
        regime: "STRONG_UPTREND", "WEAK_UPTREND", "RANGING", "WEAK_DOWNTREND", "STRONG_DOWNTREND"
        strength: 0-1 (قوة الاتجاه)
    """
    ...
    # حساب المتوسطات
    ma_short = close.tail(period_short).mean()
    ma_mid = close.tail(period_mid).mean()
    ma_long = close.tail(period_long).mean()
    
    # حساب الفروقات والنسب
    diff_short_mid = ma_short - ma_mid
    diff_mid_long = ma_mid - ma_long
    
    # حساب قوة الاتجاه (ADX-like)
    recent_highs = high.tail(period_short).max()
    recent_lows = low.tail(period_short).min()
    recent_range = recent_highs - recent_lows
    ma_range = (high.tail(period_short) - low.tail(period_short)).mean()
    
    strength = min(recent_range / ma_range if ma_range > 0 else 1.0, 2.0) / 2.0
    
    # تحديد الـ Regime
    if diff_short_mid > 0 and diff_mid_long > 0:
        if strength > 0.6:
            return "STRONG_UPTREND", strength
        else:
            return "WEAK_UPTREND", strength
    
    elif diff_short_mid < 0 and diff_mid_long < 0:
        if strength > 0.6:
            return "STRONG_DOWNTREND", strength
        else:
            return "WEAK_DOWNTREND", strength
    
    else:
        return "RANGING", strength * 0.7


def get_regime_parameters(regime):
    """
    الحصول على المعاملات حسب الـ Regime
    
    تعديل Score Threshold و Target و Risk
    """
    ...
    params_map = {
        "STRONG_UPTREND": {
            ...
            "max_discount_depth": 0.4,
            "score_boost": 0,
            "description": "📈 Strong uptrend - Easy buying",
            "color": "🟢"
        },
        "WEAK_UPTREND": {
            "score_threshold": 30,
            ...
            "score_boost": 5,
            "description": "📈 Weak uptrend - Normal conditions",
            "color": "🟡"
        },
        "RANGING": {
            "score_threshold": 35,
            "target_multiplier": 1.12,
            "max_discount_depth": 0.7,
            "score_boost": 0,
            "description": "↔️ Ranging - Standard SMC",
            "color": "🔵"
        },
        "WEAK_DOWNTREND": {
            "score_threshold": 50,
            "target_multiplier": 1.10,
            "max_discount_depth": 0.9,
            "score_boost": -20,
            "description": "📉 Weak downtrend - Cautious",
            "color": "🟠"
        },
        "STRONG_DOWNTREND": {
            "score_threshold": 70,
            "target_multiplier": 1.08,
            "max_discount_depth": 0.99,
            "score_boost": -30,
            "description": "📉 Strong downtrend - Avoid",
            "color": "🔴"
        },
        "UNKNOWN": {
            "score_threshold": 40,
    ...
    return params_map.get(regime, params_map["UNKNOWN"])
...
# =========================================
# BACKTESTING ENGINE (REGIME-AWARE)
# =========================================

class BacktestEngineRegimeAware:
    ...
    def _analyze_bar(self, idx, regime_params):
        """تحليل شمعة واحدة مع Regime-Awareness"""
        ...
        # حساب Discount Depth
        discount_range = eq - lo
        discount_depth = (eq - cur) / discount_range if discount_range > 0 else 0
        ...
        # تطبيق تعديل الـ Regime (Score Boost/Penalty)
        total_score = max(0, total_score + regime_params["score_boost"])
        ...
        # التحقق من Discount Depth
        discount_ok = discount_depth <= regime_params["max_discount_depth"]
        ...
        return {
            ...
            "target": cur * regime_params["target_multiplier"],
            "regime": regime_params["description"],
        }
    
    def run(self):
        """تشغيل الاختبار الخلفي مع Regime Detection"""
        position = None
        regime_stats = {
            "STRONG_UPTREND": {"trades": 0, "wins": 0, "pnl": 0},
            "WEAK_UPTREND": {"trades": 0, "wins": 0, "pnl": 0},
            "RANGING": {"trades": 0, "wins": 0, "pnl": 0},
            "WEAK_DOWNTREND": {"trades": 0, "wins": 0, "pnl": 0},
            "STRONG_DOWNTREND": {"trades": 0, "wins": 0, "pnl": 0},
        ...
        for idx in range(self.params["min_data_points"], len(self.df)):
            ...
            regime, regime_strength = detect_market_regime(
                self.df["Close"].iloc[:idx+1],
                self.df["High"].iloc[:idx+1],
                self.df["Low"].iloc[:idx+1],
                self.df["Volume"].iloc[:idx+1]
            )
            
            # الحصول على المعاملات الخاصة بالـ Regime
            regime_params = get_regime_parameters(regime)
            regime_params["score_threshold"] = regime_params["score_threshold"]
            
            # تحليل الشمعة
            bar_analysis = self._analyze_bar(idx, regime_params)
            ...
            discount_ok = bar_analysis["discount_ok"]
            target = bar_analysis["target"]
            regime_key = regime
            
            # BUY SIGNAL
            if not position and price_ok and discount_ok and score >= regime_params["score_threshold"]:
                position = {
                    "entry_date": date,
                    "entry_price": price,
                    "target": target,
...
def optimize_parameters_with_z3_enhanced(sample_results):
    """تحسين محسّن للمعاملات باستخدام Z3"""
    
    if not HAS_Z3:
        print("⚠️ Z3 غير متاح")
        return BacktestEngineRegimeAware._default_params()
    
    print("\n" + "="*70)
    print("🧮 تحسين المعاملات باستخدام Z3 Theorem Prover...")
    print("="*70)
    ...
    if solver.check() == sat:
        ...
        optimized = BacktestEngineRegimeAware._default_params()
        optimized.update({
...
def run_full_backtest(stocks=None):
    """تشغيل الاختبار الخلفي الشامل مع Regime Detection"""
    ...
    print("\n" + "="*70)
    print("🚀 EGX SMC SCANNER - 5-YEAR BACKTEST (REGIME-AWARE)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("🔄 تشغيل الاختبار الخلفي مع كشف أنماط السوق...")
    print("="*70 + "\n")
    
    for symbol in tqdm(data_cache.keys(), desc="Backtesting"):
        engine = BacktestEngineRegimeAware(symbol, data_cache[symbol], optimized_params)
        result = engine.run()
        all_results.append(result)
        all_trades.extend(engine.trades)
        all_regimes[symbol] = engine.regime_stats
    
    return all_results, all_trades, optimized_params, all_regimes


def generate_report(all_results, all_trades, optimized_params, all_regimes):
    """توليد التقرير الشامل"""
    
    print("\n" + "="*70)
    print("📊 BACKTEST RESULTS - 5 YEARS (REGIME-AWARE)")
    print("="*70 + "\n")
    ...
    for symbol, regime_stats in all_regimes.items():
    ...
    print(f"{'Regime':<20} {'Trades':<10} {'Win%':<10} {'Avg P&L':<12} {'Total P&L':<12}")
    print("-" * 70)
    
    for regime, stats in regime_totals.items():
        if stats["trades"] > 0:
            ...
            print(f"{color_map[regime]} {regime:<17} {stats['trades']:<10} "
                  f"{win_rate:.1f}%{'':<6} {avg_pnl:>10.2f} {stats['pnl']:>12.2f}")
    ...
    if all_trades:
        ...
        for i, (_, trade) in enumerate(trades_df.head(5).iterrows(), 1):
            print(f"{i}. {trade['symbol']}: {trade['entry_date'].date()} → {trade['exit_date'].date()} | "
                  f"+{trade['pnl']:.2f} EGP (+{trade['pnl_pct']:.2f}%) | Regime: {trade['regime']}")
    ...
    save_results(all_results, all_trades, optimized_params, all_regimes)


def save_results(all_results, all_trades, optimized_params, all_regimes):
    """حفظ النتائج"""
    
    results_df = pd.DataFrame(all_results)
    results_df.to_csv("backtest_results_regime_aware.csv", index=False)
    
    if all_trades:
        trades_df = pd.DataFrame(all_trades)
        trades_df.to_csv("backtest_trades_regime_aware.csv", index=False)
    
    with open("optimized_params_regime_aware.json", "w") as f:
        json.dump(optimized_params, f, indent=2)
    
    # حفظ إحصائيات الـ Regime
    with open("regime_statistics.json", "w") as f:
        json.dump(all_regimes, f, indent=2)
    
    print("✅ تم حفظ النتائج:")
    print("   - backtest_results_regime_aware.csv")
    print("   - backtest_trades_regime_aware.csv")
    print("   - optimized_params_regime_aware.json")
    print("   - regime_statistics.json")
...
if __name__ == "__main__":
    try:
        all_results, all_trades, optimized_params, all_regimes = run_full_backtest()
        generate_report(all_results, all_trades, optimized_params, all_regimes)
        print("\n✅ تم الانتهاء من الاختبار الخلفي مع Regime-Aware Detection بنجاح!")
        print("="*70)
