...
def _section_research_lab() -> str:
    """Research tab: Alpha Discovery Layer — hypotheses, validation stats, promotion queue."""

    STATUS_COLOR = {
        "Tracking":  "#50d8d0",   # blue
        "Improving": "#4caf50",   # green
        "Stable":    "#f0b840",   # amber
        "Failing":   "#f44336",   # red
        "Promoted":  "#4caf50",   # green
        "Rejected":  "#8b8fa8",   # dim
        "Retired":   "#8b8fa8",   # dim
    }
    ...
    def _badge_status(s):
        ...
        return (f'<span style="background:{c}22;color:{c};border:1px solid {c};'
                f'padding:2px 8px;border-radius:4px;font-size:0.78em;font-weight:600">{s}</span>')

    def _fmt_corr(v):
        if v is None:
            return '<span style="color:{DIM}">—</span>'.format(DIM=DIM)
        c = G if v >= 0.15 else (A if v >= 0.05 else R)
        return f'<span style="color:{c}">{v:+.3f}</span>'
    ...
    def _fmt_pct(v):
        ...
        return f'<span style="color:{c}">{v*100:+.1f}pp</span>'

    def _q_bar(vals):
        """Mini quartile bar: Q1 Q2 Q3 Q4 colored by value."""
        ...
        for i, v in enumerate(vals):
            lbl = f"Q{i+1}"
            if v is None:
                parts.append(f'<span style="color:{DIM};font-size:0.75em">{lbl}:—</span>')
            else:
                c = G if v >= 0.07 else (A if v >= 0 else R)
                parts.append(
                    f'<span style="color:{c};font-size:0.75em">{lbl}:{v*100:+.1f}%</span>'
                )
        return ' <span style="color:{DIM}">|</span> '.format(DIM=DIM).join(parts)

    # ── Fetch data ────────────────────────────────────────────────────────────
    hypotheses = _db_query(
        "SELECT * FROM research_hypotheses ORDER BY "
        "CASE status WHEN 'Improving' THEN 0 WHEN 'Stable' THEN 1 "
        "WHEN 'Tracking' THEN 2 WHEN 'Failing' THEN 3 "
        "WHEN 'Promoted' THEN 4 WHEN 'Rejected' THEN 5 WHEN 'Retired' THEN 6 ELSE 7 END, "
        "updated_at DESC"
    )
    recent_log = _db_query(
        "SELECT rl.*, rh.name as hyp_name FROM research_log rl "
        "JOIN research_hypotheses rh ON rl.hypothesis_id = rh.id "
        "ORDER BY rl.id DESC LIMIT 50"
    )

    if not hypotheses:
        return (
            f'<div style="background:{BG1};border:2px solid {BOR};border-radius:10px;'
            f'padding:24px;margin-bottom:18px">'
            f'{_section_header("RESEARCH LAB — ALPHA DISCOVERY", "🧪")}'
            f'<p style="color:{DIM}">No research hypotheses found. '
            f'Run <code>python research_features.py init</code> to initialise.</p></div>'
        )

    # ── Section 1: Active Hypotheses ─────────────────────────────────────────
    active = [h for h in hypotheses if h.get("status") not in ("Rejected", "Retired")]
    active_rows = ""
    for h in active:
        q_mfe = [h.get(f"q{i}_mfe_40d") for i in range(1, 5)]
        q_r20 = [h.get(f"q{i}_r20d") for i in range(1, 5)]
        active_rows += f"""
        <tr>
          <td style="padding:7px 10px;font-weight:600;color:{FG}">{h.get('name','—')}</td>
          <td style="padding:7px 10px">{_badge_status(h.get('status','—'))}</td>
          <td style="padding:7px 10px;text-align:right">{_fmt_corr(h.get('corr_mfe_40d'))}</td>
          <td style="padding:7px 10px;text-align:right">{_fmt_corr(h.get('corr_r20d'))}</td>
          <td style="padding:7px 10px;text-align:right">{_fmt_r2(h.get('incremental_r2'))}</td>
          <td style="padding:7px 10px;text-align:right">{_fmt_pct(h.get('win_rate_lift'))}</td>
          <td style="padding:7px 10px;text-align:right;color:{DIM}">{h.get('n_signals',0)}</td>
          <td style="padding:7px 10px">{_q_bar(q_mfe)}</td>
        </tr>"""
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.83em">
      <thead>
        <tr style="color:{DIM};border-bottom:1px solid {BOR}">
          ...
          <th style="padding:6px 10px;text-align:left">MFE40 by Quartile (Q1→Q4)</th>
        </tr>
      </thead>
      <tbody>{active_rows}</tbody>
    </table></div>"""

    # ── Section 2: Validation Statistics (per hypothesis detail) ─────────────
    val_cards = ""
    for h in active:
        q_mfe = [h.get(f"q{i}_mfe_40d") for i in range(1, 5)]
        q_r20 = [h.get(f"q{i}_r20d") for i in range(1, 5)]
        last_v = _ts(h.get("last_validated"), "%Y-%m-%d")
        target = h.get("promotion_target") or "—"
        desc   = h.get("description") or ""
        val_cards += f"""
        <div style="background:{BG2};border:1px solid {BOR};border-radius:6px;
                    padding:12px 16px;margin-bottom:10px">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
            <span style="font-weight:700;color:{FG}">{h.get('name','—')}</span>
            {_badge_status(h.get('status','—'))}
            <span style="color:{DIM};font-size:0.78em">target: {target}</span>
            <span style="color:{DIM};font-size:0.78em;margin-left:auto">validated: {last_v}</span>
          </div>
          <div style="color:{DIM};font-size:0.78em;margin-bottom:10px;line-height:1.5">{desc}</div>
          <div style="display:flex;flex-wrap:wrap;gap:18px;font-size:0.82em">
            <div>
              <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">CORRELATION vs MFE_40D</div>
              {_fmt_corr(h.get('corr_mfe_40d'))} &nbsp;
              <span style="color:{DIM}">vs R20D:</span> {_fmt_corr(h.get('corr_r20d'))}
            </div>
            <div>
              <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">INCREMENTAL R² vs r1-r8</div>
              {_fmt_r2(h.get('incremental_r2'))}
            </div>
            <div>
              <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">WIN RATE LIFT (Q4 vs Q1)</div>
              {_fmt_pct(h.get('win_rate_lift'))}
            </div>
            <div>
              <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">SIGNALS WITH SCORE</div>
              <span style="color:{FG}">{h.get('n_signals',0)}</span>
            </div>
          </div>
          <div style="margin-top:10px">
            <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">QUARTILE ANALYSIS — MFE_40D (Q1=lowest score → Q4=highest score)</div>
            <div style="font-size:0.82em">{_q_bar(q_mfe)}</div>
          </div>
          <div style="margin-top:6px">
            <div style="color:{DIM};font-size:0.75em;margin-bottom:4px">QUARTILE ANALYSIS — R20D</div>
            <div style="font-size:0.82em">{_q_bar(q_r20)}</div>
          </div>
        </div>"""
    ...
    overlap_rows = f"""
      <tr>
        <td style="padding:7px 10px;color:{FG}">Compression Score</td>
        <td style="padding:7px 10px;color:{DIM}">feat_atr_compression</td>
        <td style="padding:7px 10px">
          <span style="color:{G};font-weight:600">B — Alpha Amplifier</span>
        </td>
        <td style="padding:7px 10px;color:{DIM};font-size:0.8em">
          feat_atr_compression = single bar ATR ratio snapshot.
          Compression Score = directional trajectory over 10 bars (new information).
        </td>
      </tr>
      <tr style="border-top:1px solid {BOR}">
        <td style="padding:7px 10px;color:{FG}">Volume Dry-Up Score</td>
        <td style="padding:7px 10px;color:{DIM}">feat_vol_contraction</td>
        <td style="padding:7px 10px">
          <span style="color:{G};font-weight:600">B — Alpha Amplifier</span>
        </td>
        <td style="padding:7px 10px;color:{DIM};font-size:0.8em">
          feat_vol_contraction = single bar ratio. Dry-Up Score = recency-weighted
          sustained sequence across 8 bars (new information).
        </td>
      </tr>
      <tr style="border-top:1px solid {BOR}">
        <td style="padding:7px 10px;color:{FG}">Support Failure Score</td>
        <td style="padding:7px 10px;color:{DIM}">None</td>
        <td style="padding:7px 10px">
          <span style="color:{B};font-weight:600">A — New Alpha</span>
        </td>
        <td style="padding:7px 10px;color:{DIM};font-size:0.8em">
          No equivalent in r1-r8. Structural breakdown risk: distribution volume +
          lower-high formation + failed sweep recovery. Exit research only.
        </td>
      </tr>"""
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.83em">
      <thead>
        <tr style="color:{DIM};border-bottom:1px solid {BOR}">
          <th style="padding:6px 10px;text-align:left">Research Feature</th>
          <th style="padding:6px 10px;text-align:left">Nearest Existing Column</th>
    ...
    # ── Section 4: Promotion Queue ────────────────────────────────────────────
    promo_candidates = [
        h for h in hypotheses
        if h.get("status") == "Improving"
        and (h.get("corr_mfe_40d") or 0) >= 0.15
        and (h.get("incremental_r2") or 0) >= 0.01
    ]
    if promo_candidates:
        promo_rows = ""
        for h in promo_candidates:
            promo_rows += f"""
            <tr>
              <td style="padding:7px 10px;font-weight:600;color:{FG}">{h.get('name','—')}</td>
              <td style="padding:7px 10px">{_fmt_corr(h.get('corr_mfe_40d'))}</td>
              <td style="padding:7px 10px">{_fmt_r2(h.get('incremental_r2'))}</td>
              <td style="padding:7px 10px;color:{DIM}">{h.get('n_signals',0)}</td>
              <td style="padding:7px 10px;color:{DIM}">{h.get('promotion_target','—')}</td>
              <td style="padding:7px 10px;color:{DIM};font-size:0.78em">
                python research_features.py promote {h.get('id','')} {h.get('promotion_target','')}
              </td>
            </tr>"""
        promo_content = f"""
        <table style="width:100%;border-collapse:collapse;font-size:0.83em">
          <thead>
            <tr style="color:{DIM};border-bottom:1px solid {BOR}">
              ...
              <th style="padding:6px 10px;text-align:left">Promote Command</th>
            </tr>
          </thead>
        ...
        promo_content = (
            f'<p style="color:{DIM};font-size:0.83em;padding:8px 0">'
            f'No hypotheses meet promotion criteria yet '
            f'(need status=Improving, r≥0.15, ΔR²≥0.01).</p>'
        )

    # ── Section 5: Promotion History ──────────────────────────────────────────
    prom_log = [r for r in recent_log if r.get("event_type") == "Promoted"]
    if prom_log:
        prom_rows = ""
        for r in prom_log[:10]:
            det = {}
            try:
            ...
            prom_rows += f"""
            <tr>
              <td style="padding:6px 10px;color:{FG}">{r.get('hyp_name','—')}</td>
              <td style="padding:6px 10px;color:{G}">Promoted</td>
              <td style="padding:6px 10px;color:{DIM}">{det.get('target','—')}</td>
              <td style="padding:6px 10px;color:{DIM}">{_ts(r.get('logged_at'),"%Y-%m-%d")}</td>
              <td style="padding:6px 10px;color:{DIM};font-size:0.78em">{det.get('note','')}</td>
            </tr>"""
        promo_hist = f"""
        <table style="width:100%;border-collapse:collapse;font-size:0.83em">
          <thead><tr style="color:{DIM};border-bottom:1px solid {BOR}">
            <th style="padding:6px 10px;text-align:left">Hypothesis</th>
        ...
        promo_hist = (
            f'<p style="color:{DIM};font-size:0.83em;padding:8px 0">No promotions yet.</p>'
        )

    # ── Section 6: Rejected / Retired Archive ────────────────────────────────
    archived = [h for h in hypotheses if h.get("status") in ("Rejected", "Retired")]
    if archived:
        arch_rows = ""
        for h in archived:
            last_log = _db_query(
                "SELECT details_json, logged_at FROM research_log "
                "WHERE hypothesis_id = ? AND event_type IN ('Rejected','Retired') "
            ...
            if last_log:
                try:
                    note = json.loads(last_log[0].get("details_json") or "{}").get("note", "")
                except Exception:
                    pass
            arch_rows += f"""
            <tr>
              <td style="padding:6px 10px;color:{DIM}">{h.get('name','—')}</td>
              <td style="padding:6px 10px">{_badge_status(h.get('status','—'))}</td>
              <td style="padding:6px 10px;color:{DIM}">{_fmt_corr(h.get('corr_mfe_40d'))}</td>
              <td style="padding:6px 10px;color:{DIM}">{h.get('n_signals',0)}</td>
              <td style="padding:6px 10px;color:{DIM};font-size:0.78em">{note}</td>
              <td style="padding:6px 10px;color:{DIM}">{_ts(h.get('updated_at'),"%Y-%m-%d")}</td>
            </tr>"""
        archive_content = f"""
        <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:0.83em">
        ...
        archive_content = (
            f'<p style="color:{DIM};font-size:0.83em;padding:8px 0">'
            f'No rejected or retired hypotheses.</p>'
        )

    # ── Section 7: Historical Research Log ───────────────────────────────────
    log_rows = ""
    for r in recent_log[:30]:
        et = r.get("event_type", "—")
        et_color = {
            "Promoted": G, "Validated": B, "Created": DIM,
            "StatusChange": A, "Rejected": R, "Retired": DIM,
        }.get(et, DIM)
        log_rows += f"""
        <tr style="border-top:1px solid {BOR}">
          <td style="padding:5px 10px;color:{DIM};font-size:0.78em">{_ts(r.get('logged_at'),"%Y-%m-%d %H:%M")}</td>
          <td style="padding:5px 10px;color:{FG};font-size:0.82em">{r.get('hyp_name','—')}</td>
          <td style="padding:5px 10px">
            <span style="color:{et_color};font-size:0.78em;font-weight:600">{et}</span>
          </td>
          <td style="padding:5px 10px;color:{DIM};font-size:0.78em;max-width:360px;word-break:break-word">{r.get('details_json','')[:120]}</td>
        </tr>"""
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.83em">
      ...
      <tbody>{log_rows or f'<tr><td colspan="4" style="padding:10px;color:{DIM}">No events yet.</td></tr>'}</tbody>
    </table>"""

    # ── Assemble ──────────────────────────────────────────────────────────────
    return f"""
<div style="background:{BG1};border:2px solid {B};border-radius:10px;padding:16px 20px;margin-bottom:18px">
  {_section_header("RESEARCH LAB — ALPHA DISCOVERY", "🧪")}

  <div style="color:{DIM};font-size:0.78em;margin-bottom:14px;line-height:1.6">
    Alpha Discovery Layer: hypotheses are tracked, validated, and promoted through
    <strong style="color:{FG}">Tracking → Improving → Stable/Failing → Promoted/Rejected/Retired</strong>.
    Research scores are stored on signal records and validated against <strong style="color:{FG}">mfe_40d</strong>
    and <strong style="color:{FG}">r20d</strong> (validated outcome columns only).
    Exit-related scores (Support Failure) are research-only and never affect production decisions.
  </div>

  {_box("ACTIVE HYPOTHESES", active_table, B)}
  {_box("VALIDATION STATISTICS", val_cards, B)}
  {_box("OVERLAP ANALYSIS — NON-DUPLICATION CHECK", overlap_table, A)}
  {_box("PROMOTION QUEUE", promo_content, G)}
  {_box("PROMOTION HISTORY", promo_hist, G)}
  {_box("REJECTED / RETIRED ARCHIVE", archive_content, DIM)}
  {_box("HISTORICAL RESEARCH LOG (last 30 events)", log_table, DIM)}
</div>"""
