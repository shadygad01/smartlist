...
# Fall back to latest signal_history prices when scan_results.json is missing/empty
...
<div id="token-modal" onclick="if(event.target===this)closeTokenModal()">
  <div class="modal-box">
    ...
    <div id="token-error" style="display:none;color:#f85149;font-size:11px;margin-bottom:10px;padding:6px 10px;background:rgba(248,81,73,.1);border:1px solid rgba(248,81,73,.3);border-radius:6px">
      ✕ Token rejected (401/403) — paste a new valid token below
    </div>
...
const PEND_MAP  = Object.fromEntries(PENDING_LIST.map(p => [p.ticker, p]));
...
function buildHeatmap() {{
    ...
    function addStockRow(stock) {{
        ...
        const effLbl = effV === null ? '' : effV>=3?'Excellent':effV>=2?'Strong':effV>=1?'Moderate':'Weak';
...
function openTokenModal(runAfter, showError) {{
